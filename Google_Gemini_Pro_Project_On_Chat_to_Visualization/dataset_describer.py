...
if uploaded_file is not None:
    ...
    genai.configure(api_key=st.secrets['google_gen_ai_key']['API_KEY'])
    ...
    if plot_description:
        response_dataset_description = genai.generate_text(
            prompt=st.secrets['genai_queries']['datadescribe_query1'] +
                   st_version[0:200] + "\n" +
                   st.secrets['genai_queries']['datadescribe_query2'],
            max_output_tokens=max_token,
            temperature=summ_temprature
        )

        response_dataset_summary = genai.generate_text(
            prompt=st.secrets['genai_queries']['datadescribe_query1'] +
                   st_version[0:200] + "\n" +
                   plot_description + "\n" +
                   st.secrets['genai_queries']['datadescribe_query3'] +
                   st.secrets['genai_queries']['datadescribe_query4'] +
                   st.secrets['genai_queries']['datadescribe_query5'] +
                   st.secrets['genai_queries']['datadescribe_query6'] +
                   f'datasets/{unique_filename}.csv' +
                   st.secrets['genai_queries']['datadescribe_query7'],
            max_output_tokens=max_token,
            temperature=summ_temprature
        )

        with st.sidebar:
            st.write(response_dataset_description.candidates[0]['output'])

        created_code = extract_string_between_triple_backticks(response_dataset_summary.candidates[0]['output'])
